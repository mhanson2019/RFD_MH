...
def main():
    ...
    for pdb in pdbs:
        ...
        secstruc_dict=extract_secstruc(pdb, ssMan=args.ssString)
...
def get_args():
    ...
    # add options for manually defining secondary structure for more control over masking
    parser.add_argument("--ssString", dest="ssString", required=False, help="string of secondary structure, e.g. 'HHHLLEEELLLLHHH'", default=None)
    args = parser.parse_args()
    return args


def extract_secstruc(fn, ssMan=None):
    pdb=parse_pdb(fn)
    idx = pdb['idx']
    if APPROX:
        aa_sequence = pdb["seq"]
