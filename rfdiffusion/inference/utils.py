...
class BlockAdjacency:
    ...
    def __init__(self, conf, num_designs):
        ...
        # convert the code below to process lists
        if OmegaConf.is_list(self.conf.scaffoldguided.sampled_insertion): 
            self.sampled_insertion = []
            for loop in self.conf.scaffoldguided.sampled_insertion:
                if "-" in str(loop):
                    self.sampled_insertion.append([
                        int(str(loop).split("-")[0]),
                        int(str(loop).split("-")[1])
                        ])
                else:
                    self.sampled_insertion.append([0, int(loop)])
        else:
    ...
    def expand_mask(self, mask, segments):
        ...
        output = N_add * [False]
        if type(self.sampled_insertion) == list:
            i = 0
            for ss, length in segments:
                if ss == "ss":
                    ...
                    ins = random.randint(
                        self.sampled_insertion[i][0], self.sampled_insertion[i][1]
                    )
